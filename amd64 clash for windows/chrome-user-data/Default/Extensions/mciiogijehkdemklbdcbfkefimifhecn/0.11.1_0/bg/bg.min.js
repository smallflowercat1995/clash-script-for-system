...
var I,E=!1,e=!1,z=!1;(function(){var f=localStorage.ver,g=chrome.runtime.getManifest().version,m=!1,A=f||g;localStorage.ver1&&localStorage.ver1_t||(localStorage.ver1=A,localStorage.ver1_t=Date.now());if(f){A=d(f);var F=d(g);"0.11.1"!=g&&(A[2]=F[2]=0);0>l(A,F)&&(m=!0);f!=g&&(I=f)}else m=z=!0;m&&(E=!0,e&&b());localStorage.ver=g})();window.addEventListener("load",function(){Vb.open(function(){Vb.hb(function(f){for(var g=0;g<f.length;++g){var m=f[g];x.set(m.id);m.state==J&&(m.state=Ta,V.g.set(m.id),X.enqueue(m));
-1==m.cr_id&&(m.cr_id=-2)}V.a=f;Z.Yb()&&(y(),setTimeout(q,8E3))})})});window.onunload=function(){chrome.runtime.onMessage.removeListener(Xb.aa());chrome.runtime.onMessageExternal.removeListener(Xb.wa());Vb.cc();Tb.commit();Ub.commit();Wb.stop();pc.stop();U.fa(!1);Vb.commit(Vb.close)};var D=[];V.Ub=function(f){D.push(f)};var O=1;V.jc=function(f){-1!=V.j?(console.warn("closing ui tab "+V.j),chrome.tabs.remove(V.j,()=>{V.Ma(f)})):V.Ma(f)};V.Ma=function(f){V.j=f;u(function(){for(;0<D.length;)D.shift()()});
f=new XMLHttpRequest;f.open("GET","https://www.chronodownloader.net?i="+O++,!0);f.send()};V.ic=function(){V.j=-1;console.log("ui disconnected")};V.zb=function(f){function g(m){chrome.downloads.getFileIcon(m.cr_id,{size:16},function(A){chrome.runtime.lastError||V.l(m,"iconURL16",A)});chrome.downloads.getFileIcon(m.cr_id,{size:32},function(A){chrome.runtime.lastError||(V.l(m,"iconURL32",A),m.state==J&&lc(m,A))})}chrome.downloads.search({},function(m){if(m&&m.length){k();for(var A=V.a.length+10,F=0;F<
m.length;++F){var L=n(m[F]);if(L){if(-1==L){U.h({c:!0},function(){r({cmd:"u_alert",data:nb.noID})});break}Z.oa(L,m[F].id,m[F].filename,!0);L=V.ga(L,!1,!1);L<A&&(A=L)}}C();A<V.a.length&&(r({cmd:"u_new",idx:A,scroll_id:V.a[V.a.length-1].id}),m=V.a.slice(A),m.reverse(),K.add(g,m,15,!1,100))}f&&f()})};var Q,aa=0;V.u=function(f){if(!va&&!Q){var g=Date.now(),m=aa;aa=g;500>g-m?Q=setTimeout(function(){Q=void 0;V.u(-1)},500):(g="",R.get("badge_downnum")&&(m=Z.Fa(),99<m?g="99+":0<m&&(g=m.toString(),-1==f&&
(f=0<X.Kb()),f&&(g+="+"))),chrome.browserAction.setBadgeText({text:g}))}};V.B=r;var va=!1;V.A=function(f,g,m){V.g.set(f.id);f._cf={};0!=m&&V.B({cmd:"u_fields",id:f.id,fields:g})};V.l=function(f,g,m,A){f[g]=m;V.A(f,[g],A)};var W=document.createElement("canvas").getContext("2d");var c=new Image;c.src="../icons/logo/16.png";var B=0;V.Ua=H;V.qa=function(f,g){var m=R.J().addtop,A=V.ga(f,m,!g);V.B({cmd:"u_new",ids:[f.id],idx:A,addtop:m,scroll_id:f.id});U.ya(function(F){chrome.tabs.sendMessage(F.id,{cmd:"d_anim",
addpause:g})});H();R.get("notify_oneclick")&&ec(f,g);oa("fe_us","dl_src","chrome")};V.Z=function(f,g){var m=new P;m.id=x.Ea(!0);if(-1==m.id)return null;m.referer=g||"";m.eurl=m.url=f;return m};V.ga=function(f,g,m){if(!f)return-1;f.naming_auto=!f.naming;!m&&f.folder&&(m=Fa(f.folder),sa(m)&&Tb.ka(m,f.time_add));g?(V.a.unshift(f),g=0):(V.a.push(f),g=V.a.length-1);V.g.set(f.id);return g};V.$b=function(f){f||(f=V.a);var g=-1234;R.get("start_interrupted")&&(g=M);for(var m=[],A=0,F=f.length;A<F;++A){var L=
f[A];if(L.state==Ua||L.state==g)V.l(L,"state",Ta,!1),X.enqueue(L),m.push(L.id)}m.length&&(X.M(),r({cmd:"u_tasks_by_field",ids:m,field:"state"}))};V.bc=function(f){f||(f=V.a);for(var g=[],m=0,A=f.length;m<A;++m){var F=f[m];F.state==Ta?(V.l(F,"state",Ua,!1),X.eb(F),g.push(F.id)):F.state==J&&(Z.pause(F),g.push(F.id))}g.length&&(r({cmd:"u_tasks_by_field",ids:g,field:"state"}),X.V())};V.Tb=function(f){f||(f=V.a);var g=!1,m=[];k();for(var A=0,F=f.length;A<F;++A){var L=f[A];Z.erase(L,!0);L.reset();L.state==
J?Z.download(L):(L.state=Ta,X.enqueue(L),g=!0);m.push(L.id);L._cf={};V.g.set(L.id)}C();g&&X.V();r({cmd:"u_tasks",ids:m})};V.ha=function(f,g,m){f.state==Ta&&(f.state=Ua);Z.erase(f,g);x.clear(f.id);V.g.clear(f.id);Vb.Pb(f.id);g=a(f.id);-1!=g&&V.a.splice(g,1);setTimeout(function(){mc(f.id)},200);m&&r({cmd:"u_clear",src:m})};V.Rb=function(f,g,m){k();for(var A=0,F=f.length;A<F;++A){var L=f[A];L.state==Ta&&(L.state=Ua);Z.erase(L,g);x.clear(L.id);V.g.clear(L.id)}C();K.add(Vb.Qb,f,200,!0);K.add(nc,f,50,!1,
150);m&&r({cmd:"u_clear",src:m,count:1<f.length?f.length:0})};V.xa=function(f){for(var g=[],m=V.a.length-1;0<=m;--m)f(V.a[m])&&g.push(m);return g};V.Za=function(f){return V.ra(V.xa(f),!1,"bg")};V.ra=function(f,g,m){if(!f||!f.length)return 0;console.log("Begin clearing tasks");var A=!1,F=[];k();for(var L=0;L<f.length;++L){var T=f[L],S=V.a[T];A||S.state!=J&&S.state!=Ta||(A=!0);Z.erase(S,g);x.clear(S.id);V.g.clear(S.id);F.push(S.id);V.a.splice(T,1)}C();K.add(Vb.Ob,F,200,!0);K.add(mc,F,50,!1,150);r({cmd:"u_clear",
count:F.length,src:m});A&&V.u(-1);console.log(F.length+" task(s) cleared");return F.length};var G,N;V.Wb=function(f,g){chrome.alarms.clear("auto-clear-tasks",function(){R.set("autoclear_type",f);R.set("autoclear_timer",g);w(f,g)?(R.set("autoclear_lasttime",Date.now()),chrome.alarms.create("auto-clear-tasks",{delayInMinutes:g,periodInMinutes:g})):console.log("autoclear disabled")})};chrome.alarms.onAlarm.addListener(function(f){f&&"auto-clear-tasks"==f.name&&G&&(f={type:"basic",title:h("tipBtnClear"),
priority:1,message:h("statusScheduledAutoClear",[60]),contextMessage:"@"+(new Date(Date.now()+6E4)).toLocaleString(),iconUrl:"../icons/logo/48.png"},chrome.notifications.create("int_auto_clear",f,function(){}),N&&(clearTimeout(N),N=void 0),N=setTimeout(function(){if(G){var g=V.Za(G);R.set("autoclear_lasttime",Date.now());0<g&&(g={type:"basic",title:h("tipBtnClear"),priority:1,message:h("statusTasksCleared",[g]),iconUrl:"../icons/logo/48.png"},chrome.notifications.create("int_auto_clear",g,function(){}))}},
6E4))});V.reset=function(){k();for(var f=0;f<V.a.length;++f)Z.erase(V.a[f],!1,!0,!1);V.a.splice(0,V.a.length);f=localStorage.ver1;var g=localStorage.ver1_t;localStorage.clear();f&&(localStorage.ver1=f);g&&(localStorage.ver1_t=g);Vb.clear(function(){chrome.runtime.reload()})};V.Db=function(){return K.Wa()};var K=function(){function f(){m=void 0;if(g.length){var A=g[0];if(A.Va)A.ua(A.p.slice(A.K,A.K+A.Y),A.ta);else for(var F=A.K,L=F+A.Y;F<L&&F<A.La;++F)A.ua(A.p[F],A.ta);(A.K+=A.Y)>=A.La&&g.shift();
g.length?m=setTimeout(f,g[0].iv):(r({cmd:"u_bgbusy",data:!1}),console.log("work queue empty"))}}var g=[],m;return{add:function(A,F,L,T,S,ra){S=S||150;g.push({ua:A,p:F,La:F.length,Y:L,Va:T,iv:S,K:0,ta:ra});m||(m=setTimeout(f,S),r({cmd:"u_bgbusy",data:!0}))},Wa:function(){return void 0!=m||0<g.length}}}()})();V.canFileAccess=function(){return V.P};V.setPopListOpen=function(a){V.U=a};V.isPopListOpen=function(){return V.U};V.setPopSnifferTabId=function(a){V.Ia=a};V.getUITabId=function(){return V.j};
